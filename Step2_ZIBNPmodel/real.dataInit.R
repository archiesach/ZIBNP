...
lm.beta.eta <- function(dta){
  coef <- lm(eta ~ .,data=as.data.frame(dta ))$coef #lm(eta~ .- 1,data=as.data.frame(dta ))$coef # change ----
  coef[is.na(coef)] <- 0
  return(coef)
}
...
init.get.beta <- function(data){
  ...
  beta.init <- array(,c(true_parm$K,ncol(data$X)+1,(data$C))) #array(,c(true_parm$K,ncol(data$X),(data$C)))  # change ----
...
init.da.new <- function(data){
  ...
  data$init$beta.parm$da.status <- c(1,apply( matrix(data$init$beta.parm$v.ku,true_parm$K),2, function(x)  ifelse(length(unique(x))>1,2,1) ))
  data.temp <- data
...
groupwise.beta.intercept <- function(formula,kk,d){
  # drop degenrate variables
  dt <- d[d$grp==kk,]
  any<- setdiff(names(which(apply(dt,2,function(x)  length(unique(x)))==1)),"grp") 
  
  if(length(any)==0){
    glm.fit <- lme(formula , random=  ~ 1|id, data=dt)
    cof <- coef(glm.fit)[,"(Intercept)"]
  }else if(length(any)>0){
    print(paste0("Variables with degenrate values   ", paste(any,collapse="   ,")))
    dt <- dt %>% select(-all_of(any))
    cov.name <- setdiff(names(dt),c("eta","id","grp"))
    formula <- as.formula(paste("eta ~",paste(cov.name,collapse = "+"))  )
    glm.fit <- lme(formula , random=  ~ 1|id, data=dt)
    cof <- coef(glm.fit)[,"(Intercept)"]
  }
...
init.beta.intercept <- function(data){
  ...
  colnames(xx) <- c("id", "grp",(paste("X",1:ncol(data$X),sep="")))
  bet0iu.init <- array(,c(data$n,data$C))
  
  formula <- paste("eta~",paste(setdiff(colnames(xx),c("id", "grp")), collapse ="+")  )
  ...
  for(uu in 1: length(h.u)){
    ...
    if(h.u[uu]==1){
      glm.fit1 <- lme(formula, random=  ~ 1|id, data=d)
      inf.model.intercept <- coef(glm.fit1)[,"(Intercept)"]
    }
    if(h.u[uu]==2){
      glm.fit.coef <- lapply(1:true_parm$K, function(x) groupwise.beta.intercept(formula,kk=x,d) )
      inf.model.intercept <- c(unlist(glm.fit.coef))
    }
